...
make_from_baf <- function(
  ...
  if (year == 2020) {
    baf <- PL94171::pl_get_baf(
      state,
      cache_to = here(str_glue("data-raw/{state}/{state}_baf.rds"))
    )
  } else {
    baf <- get_baf_10(
      state,
      cache_to = here(str_glue("data-raw/{state}/{state}_baf_10.rds"))
    )
    if ('VTD' %in% names(baf)) {
      baf[['VTD']] <- baf[['VTD']] |>
...
get_baf_10 <- function(
  ...
  zip_url <- str_glue(
    'https://www2.census.gov/geo/docs/maps-data/data/baf/{base_name}.zip'
  )
  ...
  for (fname in files) {
    ...
    if (!is.null(geographies) && !(geogr %in% geographies)) {
      next
    }
...
read_baf_cd113 <- function(state) {
  ...
  path <- stringr::str_glue(
    'data-raw/ZZ_baf_cd113/{censable::match_fips(state)}_{censable::match_abb(state)}_CD113.txt'
  )

  read_csv(
    path,
    col_types = c(BLOCKID = 'c', CD113 = 'i')
  ) |>
    rename(
      cd_2010 = CD113
    )
...
#' @param year_shd the year for the state house districts. Defaults to `2023`.
#' @param year_ssd the year for the state senate districts. Defaults to `2023`.
#'
#' @return a data from of `to` units, with `from` columns added, ready to be joined
#' @export
#'
#' @examples
#' leg_from_baf('DE')
leg_from_baf <- function(state, to = "VTD", year_shd = 2023, year_ssd = 2023) {
  ...
  if (to == 'VTD') {
    vtd_baf <- baf::baf(state = state, year = 2023, geographies = 'VTD')[[
      1
    ]] |>
      mutate(
        BLOCKID = BLOCKID,
