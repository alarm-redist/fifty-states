###############################################################################
# Simulate plans for `CA_cd_1990`
# © ALARM Project, November 2025
###############################################################################

# Run the simulation -----
cli_process_start("Running simulations for {.pkg CA_cd_1990}")

constr <- redist_constr(map) %>%
  add_constr_grp_hinge(strength = 1.0, group_pop = vap_hisp,  total_pop = vap) %>%
  add_constr_grp_hinge(strength = 1.0, group_pop = vap_asian, total_pop = vap)
...
write_rds(plans, here("data-out/CA_1990/CA_cd_1990_plans.rds"), compress = "xz")
cli_process_done()

# Compute summary statistics -----
cli_process_start("Computing summary statistics for {.pkg CA_cd_1990}")

plans <- add_summary_stats(plans, map)
# Output the summary statistics. Do not edit this path.
save_summary_stats(plans, "data-out/CA_1990/CA_cd_1990_stats.csv")
