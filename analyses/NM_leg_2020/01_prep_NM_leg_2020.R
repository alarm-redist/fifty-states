###############################################################################
# Download and prepare data for `NM_leg_2020` analysis
...
cli_process_start("Downloading files for {.pkg NM_leg_2020}")
...
if (!file.exists(here(shp_path))) {
    ...
    d_muni <- make_from_baf("NM", "INCPLACE_CDP", "VTD", year = 2020)  |>
        ...
        select(-vtd)
    d_ssd <- make_from_baf("NM", "SLDU", "VTD", year = 2020)  |>
        transmute(GEOID = paste0(censable::match_fips("NM"), vtd),
                  ssd_2010 = as.integer(sldu))
    d_shd <- make_from_baf("NM", "SLDL", "VTD", year = 2020)  |>
        transmute(GEOID = paste0(censable::match_fips("NM"), vtd),
    ...
    ccm(nm_shp$adj, nm_shp$ssd_2020)
    ccm(nm_shp$adj, nm_shp$shd_2020)
