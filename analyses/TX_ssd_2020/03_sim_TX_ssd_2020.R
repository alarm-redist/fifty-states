###############################################################################
# Simulate plans for `TX_ssd_2020`
# © ALARM Project, September 2025
###############################################################################

# Run the simulation -----
cli_process_start("Running simulations for TX_ssd_2020")

# VRA constraints
constraints <- redist_constr(ssd_map) %>%
...
mh_accept_per_smc <- ceiling(n_distinct(ssd_map$ssd_2020)/3) + 15
...
plans <- add_summary_stats(plans, ssd_map) |>
    mutate(total_cvap = tally_var(ssd_map, cvap), .after = total_vap)

# cvap columns
cvap_cols <- names(ssd_map)[tidyselect::eval_select(starts_with("cvap_"), ssd_map)]
for (col in rev(cvap_cols)) {
    plans <- mutate(plans, {{ col }} := tally_var(ssd_map, ssd_map[[col]]), .after = vap_two)
}
