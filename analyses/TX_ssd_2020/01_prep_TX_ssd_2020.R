###############################################################################
# Download and prepare data for `TX_ssd_2020` analysis
...
cli_process_start("Downloading files for {.pkg TX_ssd_2020}")

path_data <- download_redistricting_file("TX", "data-raw/TX", year = 2020)

# download the enacted plan.
shd_shp <- tt_state_leg_lower("TX", year = 2023)
ssd_shp <- tt_state_leg_upper("TX", year = 2023)
...
if (!file.exists(here(shp_path))) {
    ...
    tx_shp <- tx_shp %>%
        mutate(ssd_2020 = as.integer(ssd_shp$SLDUST)[
            geo_match(tx_shp, ssd_shp, method = "area")],
        .after = ssd_2010) %>%
        mutate(shd_2020 = as.integer(shd_shp$SLDLST)[
            geo_match(tx_shp, shd_shp, method = "area")],
        .after = shd_2010)
