...
# mapview can't handle list-columns (adj), so make a plotting copy
md_mv <- md_shp %>%
  select(-adj)

md_mv2 <- st_zm(st_make_valid(md_mv))

centroids <- st_point_on_surface(md_mv2)

# rebuild centroids as a clean sf object with ONLY GEOID + geometry
centroids2 <- st_sf(
  GEOID = as.character(md_mv2$GEOID),
  geometry = st_geometry(centroids),
  crs = st_crs(md_mv2)
)

m  <- mapview(md_mv2, alpha.regions = 0.2)
m2 <- mapview(centroids2, label = ~GEOID, cex = 0.6)
...
left_idx <- c(
  865, 862, 861, 314, 312, 309, 307, 308, 66, 42, 27, 34,
  75, 76, 78, 87, 177, 438, 443, 457, 456
)

# if the row numbers refer to md_shp:
left_geoids <- md_shp$GEOID[left_idx]
...
right_idx <- c(
  864, 863, 860, 313, 311, 310, 317, 317, 729, 29, 26, 33,
  46, 82, 77, 84, 170, 287, 442, 465, 455
)
right_geoids <- md_shp$GEOID[right_idx]

# print all in console
cat(as.character(right_geoids), sep = "\n")
...
# Connect bay precincts (left -> right, pairwise)
...
for (k in seq_along(left_ids)) {
  ...
  if (!is.na(u) && !is.na(v)) {
    ...
    message("Skipping pair ", k, ": ",
            left_ids[k], " -> ", right_ids[k],
            " (missing in md_shp)")
  }
}
