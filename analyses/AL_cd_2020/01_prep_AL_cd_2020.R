###############################################################################
# Download and prepare data for `AL_cd_2020` analysis
...
cli_process_start("Downloading files for {.pkg AL_cd_2020}")

path_data <- download_redistricting_file("AL", "data-raw/AL")

# download the enacted plan.
url <- "https://thearp.org/documents/583/AL_CD_Enacted11042021_QjvUMkL.zip"
path_enacted <- "data-raw/AL/AL_enacted.zip"
download(url, here(path_enacted))
unzip(here(path_enacted), exdir = here(dirname(path_enacted), "AL_enacted"))
file.remove(path_enacted)
path_enacted <- "data-raw/AL/AL_enacted/AL_CD_Enacted11042021.shp"
...
if (!file.exists(here(shp_path))) {
    ...
    # add municipalities
    d_muni <- make_from_baf("AL", "INCPLACE_CDP", "VTD")  %>%
        mutate(GEOID = paste0(censable::match_fips("AL"), vtd)) %>%
        select(-vtd)
    d_cd <- make_from_baf("AL", "CD", "VTD")  %>%
    ...
    cd_shp <- st_transform(cd_shp, st_crs(al_shp))
    al_shp <- al_shp %>%
    ...
    redist.prep.polsbypopper(shp = al_shp,
